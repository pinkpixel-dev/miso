 ...
 *
 * Shared, because both sides of Miso change a sample rate. The service does it
 * on the way into a separation model, which refuses anything but 44.1 kHz, and
 * the workbench does it when you convert a file by hand. One copy means the
 * WAV you convert in the browser went through the same filter as the one the
 * job worker stages.
 *
 * The original reason it exists is the service side. The separation models
 * refuse anything but 44.1 kHz, and every take audio.cpp generates is 48 kHz.
 * The refusal is a hard error before any work starts:
 *
